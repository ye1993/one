package cn.madcoder.one.framework.idempotent.core.aop;
...
public class IdempotentAspect {
    ...
    public void afterPointCut(JoinPoint joinPoint, Idempotent idempotent) {
        ...
        // 解析 Key
        String key = keyResolver.resolver(joinPoint, idempotent);
        boolean success = idempotentRedisDAO.delete(key);
        // 解除失败，抛出异常
        if (!success) {
            log.info("[afterPointCut][方法({}) 参数({}) 幂等解除失败]", joinPoint.getSignature().toString(), joinPoint.getArgs());
            throw new ServiceException(GlobalErrorCodeConstants.UNKNOWN.getCode(), idempotent.message());
        }
    }
