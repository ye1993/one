package cn.madcoder.one.module.infra.service.codegen.inner;
...
import cn.madcoder.one.framework.common.exception.util.ServiceExceptionUtil;
import cn.madcoder.one.framework.common.pojo.CommonResult;
import cn.madcoder.one.framework.common.pojo.PageParam;
...
import cn.madcoder.one.framework.common.util.date.LocalDateTimeUtils;
import cn.madcoder.one.framework.common.util.object.ObjectUtils;
...
import cn.madcoder.one.framework.mybatis.core.dataobject.BaseDO;
...
import cn.madcoder.one.module.infra.enums.codegen.CodegenFrontTypeEnum;
...
public class CodegenEngine {
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "one-module-${table.moduleName}/" + // 顶级模块
                "one-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "one-module-${table.moduleName}/" + // 顶级模块
                "one-module-${table.moduleName}-biz/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "one-ui-${sceneEnum.basePackage}/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "one-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
