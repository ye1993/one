package cn.madcoder.one.module.infra.service.codegen;
...
import cn.madcoder.one.module.infra.controller.admin.codegen.vo.CodegenCreateListReqVO;
import cn.madcoder.one.module.infra.controller.admin.codegen.vo.CodegenUpdateReqVO;
import cn.madcoder.one.module.infra.controller.admin.codegen.vo.table.CodegenTablePageReqVO;
import cn.madcoder.one.module.infra.controller.admin.codegen.vo.table.DatabaseTableRespVO;
import cn.madcoder.one.module.infra.controller.admin.config.vo.ConfigPageReqVO;
import cn.madcoder.one.module.infra.convert.codegen.CodegenConvert;
import cn.madcoder.one.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import cn.madcoder.one.module.infra.dal.dataobject.codegen.CodegenTableDO;
import cn.madcoder.one.module.infra.dal.dataobject.config.ConfigDO;
import cn.madcoder.one.module.infra.dal.mysql.codegen.CodegenColumnMapper;
import cn.madcoder.one.module.infra.dal.mysql.codegen.CodegenTableMapper;
import cn.madcoder.one.module.infra.enums.codegen.CodegenSceneEnum;
import cn.madcoder.one.module.infra.framework.codegen.config.CodegenProperties;
import cn.madcoder.one.module.infra.service.codegen.inner.CodegenBuilder;
import cn.madcoder.one.module.infra.service.codegen.inner.CodegenEngine;
import cn.madcoder.one.module.infra.service.db.DatabaseTableService;
...
import static cn.madcoder.one.module.infra.enums.ErrorCodeConstants.*;
...
public class CodegenServiceImpl implements CodegenService {
    ...
    public List<CodegenColumnDO> getCodegenColumnListByTableId(Long tableId) {
        return codegenColumnMapper.selectList(CodegenColumnDO::getTableId, tableId).stream()
    ...
    public Map<String, String> generationCodes(Long tableId) {
        ...
        List<CodegenColumnDO> columns = codegenColumnMapper.selectList(CodegenColumnDO::getTableId, tableId).stream()
                .sorted(Comparator.comparing(CodegenColumnDO::getId))
                .collect(Collectors.toList());
    ...
    private void syncCodegen0(Long tableId, TableInfo tableInfo) {
        ...
        List<CodegenColumnDO> codegenColumns = codegenColumnMapper.selectList(CodegenColumnDO::getTableId, tableId).stream()
                .sorted(Comparator.comparing(CodegenColumnDO::getId))
                .collect(Collectors.toList())
        ;
        ...
        //需要修改的字段
        Set<String> modifyFieldNames = tableFields.stream().filter(
                tableField -> codegenColumnDOMap.get(tableField.getColumnName()) != null
