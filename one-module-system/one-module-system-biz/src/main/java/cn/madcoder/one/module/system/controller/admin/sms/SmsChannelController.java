...
import cn.madcoder.one.module.system.controller.admin.sms.vo.channel.*;
...
import cn.madcoder.one.module.system.service.sms.SmsChannelService;
...
public class SmsChannelController {
    ...
    @GetMapping("/get")
    @Operation(summary = "获得短信渠道信息")
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('system:sms-channel:query')")
    public CommonResult<SmsChannelRespVO> getSimpleSmsChannel(@RequestParam("id") Long id) {
        SmsChannelDO smsChannelDO = smsChannelService.getSmsChannelById(id);
        return success(SmsChannelConvert.INSTANCE.convert(smsChannelDO));
    }


    @PostMapping("/create")
    @Operation(summary = "创建短信渠道信息")
